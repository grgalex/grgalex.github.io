...
Personal website of Georgios Alexopoulos, served at <https://grgalex.gr>.
Built with [Jekyll](https://jekyllrb.com/) on the [al-folio](https://github.com/alshedivat/al-folio)
theme, using <https://github.com/theosotr/theosotr.github.io> as the base. The generic
al-folio documentation follows below this section (and in `CUSTOMIZE.md`, `FAQ.md`, `INSTALL.md`).
...
- Node.js (only for `purgecss`, which the deploy workflow also runs)
- ImageMagick (`convert`), used to generate responsive `.webp` images
- `make`, `python3`

On macOS: `brew install ruby node imagemagick && gem install bundler`.

### Makefile targets

| Target         | What it does |
| -------------- | ------------ |
| `make build`   | Builds the site **exactly** as `.github/workflows/deploy.yml` does: production Jekyll build + `purgecss`, into `_site/`. That folder is byte-for-byte what gets pushed to `gh-pages`. Takes a few minutes. |
| `make serve`   | Serves the existing `_site/` at <http://localhost:4000> (no rebuild). |
| `make preview` | `make build` followed by `make serve`. |
| `make clean`   | Removes `_site/` and Jekyll caches. |

Ruby gems are installed into `vendor/bundle` automatically on the first build.
Typical loop: edit, `make build`, `make serve`, check in the browser, commit, push.
**Do not push to `main` to test something**; `make preview` shows the production output locally.

### Deployment

Pushing to `main` triggers the "Deploy site" GitHub Action, which runs the same build and
force-pushes `_site/` to the `gh-pages` branch. GitHub Pages serves `gh-pages` at `grgalex.gr`
(`CNAME` is copied into the build). Nothing else needs to be done.

### Where the content lives

| Content                     | File(s) |
| --------------------------- | ------- |
| About text, subtitle, photo | `_pages/about.md` (photo: `assets/img/me.jpg`) |
| News items                  | `_news/*.md` (one file each, `date:` in front matter, newest shown first) |
| Publications                | `_bibliography/papers.bib`. `selected={true}` puts an entry on the home page; the order there is the order of entries in the file. `pdf=`, `code=`, `arxiv=` become buttons. |
| PDFs (papers, CV)           | `assets/pdf/` (the CV is `grgalex_cv.pdf`, linked from the about page) |
| Talks                       | `_pages/talks.md` (one bullet per talk, see the comment in the file) |
| Program committees          | `_service/*.md` (rendered as "PC Member" on the home page) |
| Blog posts                  | `_posts/YYYY-MM-DD-title.md` |
| Social links                | `_data/socials.yml` |
| Site settings               | `_config.yml`, `_data/venues.yml`, `_data/coauthors.yml` |

### Deliberate deviations from the al-folio / theosotr base

- **No footer.** The footer include is commented out in `_layouts/default.liquid` and `_layouts/distill.liquid`.
- **No JavaScript.** `scripts.liquid` is not included, `assets/js/` is excluded from the build,
  `_scripts/` is not published, and all JS-backed features are switched off in `_config.yml`.
  The navbar is always expanded (`navbar-expand`) so it needs no collapse script. Ordinary blog
  posts stay script-free; features like MathJax or comments in a post would pull scripts for that page.
- **Dark theme only.** `data-theme="dark"` is set statically on `<html>`; `enable_darkmode: false`.
  `.table { color: var(--global-text-color) }` in `_sass/_base.scss` replaces the script that used to fix table colours.
- **Extra pieces:** `_includes/service.liquid` + the `service` collection; the `talks` page; Blog tab enabled.

When syncing with the base again, diff against theosotr's repository rather than upstream al-folio,
and re-apply the items above.
